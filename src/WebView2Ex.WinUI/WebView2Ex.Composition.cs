...
partial class WebView2Ex
{
    ...
    async Task<(Microsoft.UI.Composition.Visual MUXVisual, Visual WUXVisual)> CreateVisual()
    {
        try
        {
            ...
            var roundedRectangle = WUXCompositor.CreateRoundedRectangleGeometry();
            roundedRectangle.CornerRadius = new((float)CornerRadius.TopLeft);

            var spriteShape = WUXCompositor.CreateSpriteShape(roundedRectangle);
            spriteShape.FillBrush = WUXCompositor.CreateColorBrush(Microsoft.UI.Colors.Black);

            var spriteShapeVisual = WUXCompositor.CreateShapeVisual();
            spriteShapeVisual.BorderMode = CompositionBorderMode.Soft;
            spriteShapeVisual.Shapes.Add(spriteShape);

            var surface = WUXCompositor.CreateVisualSurface();
            surface.SourceVisual = spriteShapeVisual;

            var WUXVisual = WUXCompositor.CreateContainerVisual();

            var source = WUXCompositor.CreateVisualSurface();
            source.SourceVisual = WUXVisual;

            var maskBrush = WUXCompositor.CreateMaskBrush();
            maskBrush.Source = WUXCompositor.CreateSurfaceBrush(source);
            maskBrush.Mask = WUXCompositor.CreateSurfaceBrush(surface);

            var targetVisual = WUXCompositor.CreateSpriteVisual();
            targetVisual.Brush = maskBrush;

            WUXTarget.Root = targetVisual;
